...
class DataSide(backup.SourceStruct):
    ...
    def compare_hash(cls, repo_iter):
        ...
        def hashes_changed(src_rp, mir_rorp):
            ...
            if not verify_sha1:
                log.Log("Warning: Metadata file has no digest for {rp!s}, "
...
def Verify(mirror_rp, inc_rp, verify_time):
    ...
    for repo_rorp in repo_iter:
        ...
        if not verify_sha1:
            log.Log("Warning: Cannot find SHA1 digest for file {rp!s},\n"
        ...
        if computed_hash == verify_sha1:
            log.Log("Verified SHA1 digest of {rp!s}".format(rp=repo_rorp), 5)
        else:
            bad_files += 1
            log.Log("Warning: Computed SHA1 digest of {rp!s}\n   {chsh}\n"
...
def _log_success(src_rorp, mir_rorp=None):
    ...
    log.Log("Successful compare: {path}".format(path), 5)
